...
public class LPiece extends Shape{
	...
	public Block[] makeBlocks()
	{
		...
		for(int x = 0; x < 3; x++)
		{
			blocks[x] = new Block("orange");
			blocks[x].setLocation((x * 20)+20, 100);
		}
		
		blocks[3] = new Block("orange");
		blocks[3].setLocation(20, 120);
	...
	public int addToPlayingBoard(TetrisModel model, int x, int y)
	{
		...
		blocks[3].setLocation(60, 20);
		if(this.model.addBlock(blocks[3], xPos, 1+yPos) == -1)
		{
			return -1;
		}
	...
	public int rotate()
	{
		...
		if(state == dir.DOWN)
		{
			if(model.isOpen(xPos, yPos-1) && model.isOpen(xPos+1, yPos-1) && model.isOpen(xPos+1, yPos+1))
			{
				model.shift(blocks[0], 0, -1);
				model.shift(blocks[1], 0, -1);
		...
		{
			if(model.isOpen(xPos+1, yPos-1) && model.isOpen(xPos+1, yPos+1) && model.isOpen(xPos+2, yPos+1))
			{
				model.shift(blocks[0], -1, 0);
				model.shift(blocks[3], 0, 1);
				model.shift(blocks[2], 0, 1);
		...
		{
			if(model.isOpen(xPos, yPos) && model.isOpen(xPos+2, yPos) && model.isOpen(xPos, yPos+1))
			{
				model.shift(blocks[0], -1, 1);
				model.shift(blocks[2], 1, -1);
				model.shift(blocks[3], -2, 0);
				state = dir.DOWN;
			}
		}
		return 0;
